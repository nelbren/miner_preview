...
def asciichartpy():
    ...
    unpaids = Unpaid.select().where( (Unpaid.source == source) & (Unpaid.currency == currency)).order_by(Unpaid.timestamp.desc()).limit(records)
    print(unpaids)
    values = []
    usds = []
    for unpaid in reversed(unpaids):
         ...
         print(unpaid)
    test = [random.randint(1, 15) * cos(i * ((pi * 4) / width)) for i in range(width)]
    print(test)
    print(values)
    config = {
        'colors': [
...
def show_chart(source, currency):
    ...
    plt.title(f"Mining {currency.upper()} at {source.upper()} represented in USD")

    plt.subplot(2, 1)
    plt.title(f"Mining {currency.upper()} at {source.upper()} represented in BTC")
    plt.clc()
